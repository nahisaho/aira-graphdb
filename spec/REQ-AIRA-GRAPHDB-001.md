...
IF manifest で許可されていない拡張句または手続き（`CALL/APOC` を含む）を受信した場合, THEN THE システム SHALL 部分実行を行わず `UNSUPPORTED_FEATURE` を返す。
...
- Neo4j/APOC の全面互換（本版では manifest で定義するサブセットのみ対象）
- 多テナント課金機能
