...
対象ユーザーは AIRA エコシステムの他サービス開発者であり、P0 は Property Graph CRUD / Cypher 互換クエリ / 永続化 / Python・Node SDK に加え、v0.3.0 でベクトル格納最適化、ANN 検索、Cypher relationship type filter、増分永続化、entity vectors 拡張を含む。
...
**トレーサビリティ**: DES-AGDB-002, DES-AGDB-003  
**パッケージ**: `packages/aira-graphdb`  
...
**トレーサビリティ**: DES-AGDB-004, DES-AGDB-005, DES-AGDB-014  
**パッケージ**: `packages/aira-graphdb`  
...
**要件**:  
THE システム SHALL NOT `AGDB-CYPHER-OPENCYPHER9@1.0.0` の準拠範囲内クエリ（manifest で許可された `CALL/APOC` サブセットを含む）に対して `UNSUPPORTED_FEATURE` を返す。  
IF manifest で許可されていない拡張句または手続き（`CALL/APOC` と `EXISTS {}` / `CALL {}` サブクエリを含む）を受信した場合, THEN THE システム SHALL 部分実行を行わず `UNSUPPORTED_FEATURE` を返す。
...
**トレーサビリティ**: DES-AGDB-004, DES-AGDB-014, DES-AGDB-016  
**パッケージ**: `packages/aira-graphdb`  
...
**トレーサビリティ**: DES-AGDB-006, DES-AGDB-017  
**決定根拠**: ADR-AGDB-002  
**パッケージ**: `packages/aira-graphdb`  
...
IF `vector_search_mode=exact_compat` で同一データセット・同一クエリを baseline backend（neo4j）と比較した場合, THEN THE システム SHALL `topK` 結果の documentId 集合一致率を 1.0（threshold 指定時は threshold 適用後集合一致）にする。  
...
IF `vector_search_mode=hnsw_ann` の場合, THEN THE システム SHALL REQ-AGDB-027 の Recall/Latency 要件を満たす。
...
- [ ] `vector_search_mode=exact_compat` で baseline backend（neo4j）との同一クエリ比較で `topK` 結果集合が一致する
- [ ] `vector_search_mode=exact_compat` で baseline backend（neo4j）との同一クエリ比較で、`threshold` 指定時は `threshold` 適用後の `topK` 結果集合が一致する
- [ ] `vector_search_mode=hnsw_ann` の場合は REQ-AGDB-027 の評価条件で合否判定する
...
### REQ-AGDB-022: openCypher 関係走査パターン互換

**種別**: EVENT-DRIVEN  
**優先度**: P0

**要件**:  
WHEN クエリが関係走査パターン `()-[]->()`（方向あり/なし、単一ホップ）を含む場合, THE システム SHALL openCypher 9 のパターン評価規則に従って走査し、`MATCH/OPTIONAL MATCH/WHERE/RETURN` の各句で一貫した結果を返す。  
WHEN 関係走査パターンと `WITH/ORDER BY/SKIP/LIMIT` を組み合わせる場合, THE システム SHALL 句順序・スコープ・結果順序の規則を保持したまま実行する。

**受入基準**:
- [ ] `spec/conformance/rel-traversal-required.v1.0.0.yaml` の必須ケースIDで `MATCH (n)-[r]->(m)` の `expected_rows` と一致する
- [ ] `spec/conformance/rel-traversal-required.v1.0.0.yaml` の必須ケースIDで `MATCH (n)-[r]-(m)` の `expected_rows` と一致する
- [ ] `OPTIONAL MATCH` 併用ケースで `expected_rows_with_nulls` と一致する
- [ ] `ORDER BY` ありケースは `expected_order` と完全一致する
- [ ] `ORDER BY` なしケースは `multiset` 一致を満たす
- [ ] syntax/semantic エラー時に `AGDB-ERROR-CODES@1.0.0` の固定コードを返す

**トレーサビリティ**: DES-AGDB-014  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- query "MATCH (n)-[r]->(m) RETURN n,r,m"`

---

### REQ-AGDB-023: CALL/APOC 互換サブセット実装

**種別**: COMPLEX  
**優先度**: P0

**要件**:  
WHEN クエリが `CALL` 句を含む場合, THE システム SHALL `spec/contracts/apoc-procedure-manifest.v1.0.0.yaml` に列挙された手続きIDのみを実行し、未列挙手続きは部分実行せず `UNSUPPORTED_FEATURE` を返す。  
WHEN procedure 名が `spec/contracts/apoc-procedure-manifest.v1.0.0.yaml` の採用対象ID（例: `apoc.meta.schema`, `apoc.coll.toSet`, `apoc.text.join`）に一致する場合, THE システム SHALL 入出力スキーマと失敗時エラーコードを manifest 契約どおりに返す。  
IF procedure が副作用を持つ場合, THEN THE システム SHALL トランザクション境界内で原子的に実行し、失敗時に部分更新を残さない。

**受入基準**:
- [ ] `spec/contracts/apoc-procedure-manifest.v1.0.0.yaml` に列挙された procedure ID の必須ケースで `CALL` 実行が 100% 成功する
- [ ] procedure manifest（許可一覧）外の呼び出しを `UNSUPPORTED_FEATURE` で拒否する
- [ ] APOC 互換サブセットの契約テストを 100% PASS する
- [ ] 副作用手続きの失敗時に部分更新が残らない
- [ ] procedure 失敗時に固定エラーコードを返す

**トレーサビリティ**: DES-AGDB-014  
**パッケージ**: `packages/aira-graphdb`, `spec/contracts`  
**CLI**: `cargo run -p aira-graphdb -- query "CALL apoc.meta.schema()"`

---

### REQ-AGDB-024: Neo4j互換 Cypher 拡張実行

**種別**: COMPLEX  
**優先度**: P0

**要件**:  
WHEN `spec/contracts/agdb-cypher-neo4j-compat.v1.0.0.json` に列挙された Cypher 拡張を含むクエリを受信した場合, THE システム SHALL Neo4j 互換の意味規則に従って実行し、同一DB状態・同一クエリに対して決定的な結果を返す。  
IF クエリが baseline に含まれない Neo4j 固有拡張を含む場合, THEN THE システム SHALL 部分実行を行わず `UNSUPPORTED_FEATURE` を返す。

**受入基準**:
- [ ] `spec/contracts/agdb-cypher-neo4j-compat.v1.0.0.json` に列挙された機能を実行できる
- [ ] `CALL` サブクエリ、`UNION`、`FOREACH`、`CASE`、`EXISTS`、variable-length path、shortest path、schema/index 変更のうち baseline に含まれる項目を正しく処理できる
- [ ] baseline 外の Neo4j 固有拡張で部分更新が発生しない
- [ ] baseline 外の機能で `details.unsupported_clause` を含む `UNSUPPORTED_FEATURE` を返す

**トレーサビリティ**: DES-AGDB-004, DES-AGDB-014, DES-AGDB-016  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- query "<cypher>"`

---

### REQ-AGDB-025: Neo4j 互換ベースライン固定

**種別**: COMPLEX  
**優先度**: P0

**要件**:  
WHEN Neo4j 互換ベースラインを更新する場合, THE システム SHALL `spec/conformance/cypher-neo4j-compat-required.yaml` を生成し、各サポート機能を `covers_req` と `covers_acceptance` に紐づける。  
IF 互換マニフェスト、実装、または回帰テストが乖離する場合, THEN THE システム SHALL リリース判定を失敗させる。

**受入基準**:
- [ ] `spec/conformance/cypher-neo4j-compat-required.yaml` が存在する
- [ ] 必須機能集合が機械可読で、スナップショット参照を持つ
- [ ] 各必須テストに `covers_req` と `covers_acceptance` が定義される
- [ ] マニフェストと回帰テストの乖離時に CI が失敗する
- [ ] 互換性レポートが保存される

**トレーサビリティ**: DES-AGDB-006, DES-AGDB-017  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo test --test cypher_compat_conformance -- --nocapture`

---

### REQ-AGDB-026: ベクトル本体の外部バイナリ分離

**種別**: COMPLEX  
**優先度**: P0

**要件**:  
WHEN ベクトルデータを永続化する場合, THE システム SHALL JSON 内の `f64[]` を外部バイナリベクトルフォーマットへ分離し、メタデータのみをスナップショット本体に保持する。  
WHEN 容量比較プロファイル `P0-VECTOR-STORAGE-SIZE-BENCH@1.0.0`（固定データセット、固定計測時点、WAL/checkpoint 条件固定）で v0.2.x 形式と比較する場合, THE システム SHALL DB 総容量（スナップショット + 外部ベクトル + WAL）を旧形式比で 66%以上削減する。

**受入基準**:
- [ ] ベクトル本体が外部バイナリファイルへ保存される
- [ ] スナップショット本体はベクトル参照メタデータのみ保持する
- [ ] `P0-VECTOR-STORAGE-SIZE-BENCH@1.0.0` 条件下で DB 総容量（スナップショット + 外部ベクトル + WAL）が 66%以上削減される
- [ ] クラッシュ注入試験（書込み途中/COMMIT直後）で孤立バイナリファイルと dangling 参照が発生しない
- [ ] 再起動後も参照切れなくベクトル検索できる

**トレーサビリティ**: DES-AGDB-018  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- db size-report --file <path>`

---

### REQ-AGDB-027: HNSW インデックスによる ANN 検索

**種別**: STATE-DRIVEN  
**優先度**: P0

**要件**:  
IF `vector_search_mode=exact_compat` の場合, THEN THE システム SHALL exact brute-force 実行を用い、REQ-AGDB-020 の baseline 集合一致要件を満たす。  
WHILE ベンチマークプロファイル `P0-ANN-HNSW-BENCH@1.0.0`（10万件以上、同一次元、同一距離関数、同一ハードウェア、同時接続8、topK=10）で `vector_search` を HNSW モードで実行中, THE システム SHALL exact brute-force 基準実装比で検索レイテンシ（P95）を 50%以上短縮する。  
WHILE 同一ベンチマークプロファイルで HNSW モードを実行中, THE システム SHALL ground truth を exact brute-force 結果として算出した Recall@10 を 0.95 以上に維持する。

**受入基準**:
- [ ] 10万件以上ベクトルで HNSW インデックスを構築できる
- [ ] `vector_search_mode=exact_compat` で REQ-AGDB-020 の集合一致要件を満たす
- [ ] `P0-ANN-HNSW-BENCH@1.0.0` 条件下で `vector_search(topK=10)` の P95 が exact brute-force 比で 50%以上短縮される
- [ ] 同条件評価で Recall@10 が 0.95 以上（ground truth は exact brute-force 結果）
- [ ] 評価で使用する距離関数、評価クエリ集合、集計方法（平均 Recall@10）を固定定義する
- [ ] HNSW 無効時は brute-force へフォールバックできる

**トレーサビリティ**: DES-AGDB-019  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- bench ann-hnsw`

---

### REQ-AGDB-028: Cypher relationship type filter 拡張

**種別**: EVENT-DRIVEN  
**優先度**: P0

**要件**:  
WHEN クエリが関係型フィルタを含むパターン `MATCH (n)-[r:TYPE]->(m)` を受信した場合, THE システム SHALL `TYPE` に一致する関係のみを走査対象として解析・実行する。  
IF 指定した関係型が存在しない場合, THEN THE システム SHALL 空結果を返し、エラーとしない。

**受入基準**:
- [ ] `MATCH (n)-[r:TYPE]->(m)` をパースできる
- [ ] 型一致関係のみを返し、他タイプを含めない
- [ ] 型未存在時は空結果を返す
- [ ] `OPTIONAL MATCH/WHERE/RETURN` 併用時も型フィルタが維持される
- [ ] v0.3.0 で追加対象とする relationship type filter 文法を `-[r:TYPE]->` と明示し、既存 openCypher 準拠要件と矛盾しない

**トレーサビリティ**: DES-AGDB-020  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- query "MATCH (n)-[r:TYPE]->(m) RETURN n,r,m"`

---

### REQ-AGDB-029: WAL ベース増分永続化

**種別**: EVENT-DRIVEN  
**優先度**: P0

**要件**:  
WHEN トランザクションを `COMMIT` する場合, THE システム SHALL 通常 COMMIT 経路で全データ flush ではなく WAL ベースの差分書き込みを行い、差分の durable 化完了後に成功応答を返す。  
WHEN リカバリを実行する場合, THE システム SHALL WAL 差分を順序どおりに再生してスナップショットと整合する最終状態を復元する。

**受入基準**:
- [ ] `COMMIT` ごとに全スナップショット再書込みを行わない
- [ ] WAL には差分レコードのみが追記される
- [ ] クラッシュ復旧で差分再生後の状態が `COMMIT` 境界と一致する
- [ ] 部分反映禁止（REQ-AGDB-008）を維持する
- [ ] checkpoint/compaction 実行時のみフルスナップショット再生成を許容し、通常 COMMIT 経路と明確に分離される
- [ ] checkpoint/compaction なしの連続 100 回 `COMMIT` で snapshot ファイルの mtime/サイズが不変かつ WAL のみ増加する

**トレーサビリティ**: DES-AGDB-021  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- db recover --file <path>`

---

### REQ-AGDB-030: Entity vectors 64K 追加と検索品質向上

**種別**: EVENT-DRIVEN  
**優先度**: P0

**要件**:  
WHEN entity vectors の拡張投入を実行する場合, THE システム SHALL 追加 64K ベクトルを既存コーパスに取り込み、検索インデックスへ反映する。  
WHEN `vector_search_mode=hnsw_ann` かつ評価プロファイル `P0-ENTITY-VECTOR-QUALITY-BENCH@1.0.0`（固定評価セット、固定距離関数、固定クエリ集合、ground truth は exact brute-force）で拡張前後を比較する場合, THE システム SHALL Recall@10 を悪化させない。

**受入基準**:
- [ ] 追加 64K entity vectors を取り込み完了できる
- [ ] 取り込み後に検索対象件数へ反映される
- [ ] `P0-ENTITY-VECTOR-QUALITY-BENCH@1.0.0` 条件下で Recall@10 が非劣化（悪化しない）
- [ ] 上記品質評価は `vector_search_mode=hnsw_ann` の固定パラメータ（topK/距離関数/クエリ集合）で実行される
- [ ] 取り込み失敗時は部分投入を残さずロールバックできる

**トレーサビリティ**: DES-AGDB-022  
**パッケージ**: `packages/aira-graphdb`  
**CLI**: `cargo run -p aira-graphdb -- vectors import-entities --count 64000`
...
### REQ-AGDB-NF-006: 実ランタイム 24h soak 実行保証

**種別**: STATE-DRIVEN  
**優先度**: P0

**要件**:  
WHILE 連続稼働プロファイル `P0-NATIVE-SOAK` を実行中, THE システム SHALL 実ランタイム sidecar プロセスを 24 時間稼働させ、read/write 混在トラフィックと異常系注入を同時に処理する。  
WHEN CI eventType が `pull_request` の場合, THE システム SHALL `P0-NATIVE-SOAK-SMOKE`（30分）を必須実行する。  
WHEN CI eventType が `schedule` または `release` の場合, THE システム SHALL `P0-NATIVE-SOAK`（24時間）を必須実行する。

**受入基準**:
- [ ] `P0-NATIVE-SOAK` で実プロセスを 24 時間稼働させる
- [ ] `P0-NATIVE-SOAK-SMOKE` と `P0-NATIVE-SOAK` がイベント種別ごとに切り替わる
- [ ] soak 実行中に `crashCount == 0` を維持する
- [ ] 実測レポート（実行時間、リクエスト総数、障害率）を artifact として保存する

**トレーサビリティ**: DES-AGDB-015  
**パッケージ**: `packages/aira-graphdb`, `.github/workflows`  
**CLI**: `cargo run -p aira-graphdb -- soak native-rw --hours 24`

---

### REQ-AGDB-NF-007: 外部 Watchdog による kill-level 異常終了追跡

**種別**: COMPLEX  
**優先度**: P0

**要件**:  
WHEN sidecar が `SIGKILL` などプロセス内 hook で捕捉不能な終了を起こした場合, THE システム SHALL 外部 watchdog プロセスで終了を検知し、監査ログへ `errorCode`, `timestamp`, `processExitCode`, `signal`, `lastRequestId`, `uptimeSec`, `cause` を記録する。  
IF watchdog が異常終了イベントを記録した場合, THEN THE システム SHALL 次回ゲート実行時に crash reason を artifact として集約し、品質判定に反映する。

**受入基準**:
- [ ] `SIGKILL` 注入時に watchdog が `PROCESS_CRASH` を記録する
- [ ] `lastRequestId` と `uptimeSec` を復元して記録する
- [ ] crash reason が artifact に集約される
- [ ] 監査ログ必須項目欠落時に品質ゲートが失敗する

**トレーサビリティ**: DES-AGDB-015  
**パッケージ**: `packages/aira-graphdb`, `.github/workflows`, `tests`  
**CLI**: `cargo test --test native_watchdog -- --nocapture`
...
| REQ-AGDB-022 | DES-AGDB-014 |
| REQ-AGDB-023 | DES-AGDB-014 |
| REQ-AGDB-024 | DES-AGDB-004, DES-AGDB-014, DES-AGDB-016 |
| REQ-AGDB-025 | DES-AGDB-006, DES-AGDB-017 |
| REQ-AGDB-026 | DES-AGDB-018 |
| REQ-AGDB-027 | DES-AGDB-019 |
| REQ-AGDB-028 | DES-AGDB-020 |
| REQ-AGDB-029 | DES-AGDB-021 |
| REQ-AGDB-030 | DES-AGDB-022 |
...
| REQ-AGDB-NF-006 | DES-AGDB-015 |
| REQ-AGDB-NF-007 | DES-AGDB-015 |

## 5. スコープ外（v0.3.0）

- 分散クラスタの本実装（リーダー選出、再配置、自動フェイルオーバー）
- Neo4j の運用機能全般（クラスタ、マルチDB、管理ツール）の全面互換
- 多テナント課金機能
- LadybugDB 固有 SQL/Cypher 拡張の完全互換（v0.3.0 でも対象外）
