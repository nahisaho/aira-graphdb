...
| **作成日** | 2026-06-20 |
| **更新日** | 2026-06-22 |
| **要件参照** | `REQ-AIRA-GRAPHDB-001` (v2.0) |
| **設計参照** | `DES-AIRA-GRAPHDB-001`, `DES-AGDB-018`, `DES-AGDB-019`, `DES-AGDB-020`, `DES-AGDB-021`, `DES-AGDB-022` |
...
- [ ] DES カバレッジ 100%
- [ ] 依存関係 DAG（循環依存なし）
- [ ] 全タスク 2.5時間以内（v0.3.0 Delta での最大見積は TASK-AGDB-056）
...
## 12. Phase 3 追加タスク分解（v0.3.0 Storage/ANN/Cypher Delta）

### 12.1 実行順序（DAG / v0.3.0 Delta）

**前提**: TASK-AGDB-043（Native read index/検索最適化）が完了済みであること。

```mermaid
flowchart TD
  T005[TASK-AGDB-005 WAL書込みと復旧]
  T006[TASK-AGDB-006 トランザクションマネージャ]
  T021[TASK-AGDB-021 Lexer/Parser full grammar]
  T022[TASK-AGDB-022 Binder/Scope/Type checker]
  T023[TASK-AGDB-023 Planner/Optimizer rule拡張]
  T043[TASK-AGDB-043 Native read index/検索最適化]
  T054[TASK-AGDB-054 Binary vector blob store]
  T055[TASK-AGDB-055 Incremental WAL commit/replay]
  T056[TASK-AGDB-056 HNSW index + mode boundary]
  T057[TASK-AGDB-057 Cypher -[r:TYPE]-> parser/plan]
  T058[TASK-AGDB-058 Entity vectors +64K ingest]
  T059[TASK-AGDB-059 Benchmark/gate profiles]
  T060[TASK-AGDB-060 Integration/conformance wiring]
...
### 12.2 タスク一覧（v0.3.0 Delta）

### TASK-AGDB-054: 外部バイナリベクトル格納実装

**トレーサビリティ**: REQ-AGDB-026 → DES-AGDB-018  
**パッケージ**: `src/storage.rs`, `src/graph.rs`  
**種別**: backend  
**優先度**: P0  
**依存**: TASK-AGDB-005  
**見積**: 2h
...
**実装内容**:
- `VectorBlobStore` を実装し、snapshot から `f64[]` 本体を分離する
- `vectorId -> blobRef` メタデータ参照を snapshot に保持する
- `P0-VECTOR-STORAGE-SIZE-BENCH@1.0.0` 計測コマンドを実装する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] `P0-VECTOR-STORAGE-SIZE-BENCH@1.0.0`（固定 dataset_hash / seed / run_count）で DB総容量（snapshot + blob + WAL）が旧形式比で 66%以上削減される
- [ ] 書込み途中/COMMIT直後クラッシュ注入でも dangling 参照が発生しない

---

### TASK-AGDB-055: 増分 WAL COMMIT/復旧実装

**トレーサビリティ**: REQ-AGDB-029, REQ-AGDB-007, REQ-AGDB-008 → DES-AGDB-021  
**パッケージ**: `src/storage.rs`, `src/tx.rs`  
**種別**: backend  
**優先度**: P0  
**依存**: TASK-AGDB-005, TASK-AGDB-006  
**見積**: 2h

**実装内容**:
- 通常 COMMIT 経路を `delta WAL append + fsync` に固定する
- replay 順序を `globalLsn` / `txCommitMarker` に基づいて実装する
- checkpoint/compaction 経路を通常 COMMIT 経路から分離する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] checkpoint なし連続 100 COMMIT で snapshot mtime/size 不変かつ WAL のみ増加する
- [ ] クラッシュ復旧後に部分反映が発生しない

---

### TASK-AGDB-056: HNSW ANN と検索モード境界実装

**トレーサビリティ**: REQ-AGDB-027, REQ-AGDB-020 → DES-AGDB-019, DES-AGDB-010  
**パッケージ**: `src/native_bench.rs`, `src/server.rs`, `src/query.rs`  
**種別**: backend  
**優先度**: P0  
**依存**: TASK-AGDB-054, TASK-AGDB-055, TASK-AGDB-043  
**見積**: 2.5h

**実装内容**:
- `request_mode=exact_compat|hnsw_ann` を API 契約に追加し、`resolved_mode/fallback_reason` 契約キーを固定する
- `hnsw_ann` 実装と、不可時 `exact_compat` 強制フォールバックを実装する
- `exact_compat` は exact brute-force 実行で固定する
- `P0-ANN-HNSW-BENCH@1.0.0` のプロファイル定義（固定dataset/hash/seed/parameter）を実装する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] `P0-ANN-HNSW-BENCH@1.0.0` で Recall@10 >= 0.95 を満たす
- [ ] `P0-ANN-HNSW-BENCH@1.0.0` で brute-force 比 P95 50%以上短縮を満たす
- [ ] `exact_compat` では baseline backend（neo4j）との集合一致要件を維持する
- [ ] `request_mode=hnsw_ann` かつ HNSW 利用不可時、`resolved_mode=exact_compat` と `fallback_reason` を応答または監査ログで機械検証できる
- [ ] `request_mode=hnsw_ann` かつ HNSW 利用可能時、`resolved_mode=hnsw_ann` を維持し silent fallback しない
- [ ] `request_mode=exact_compat` の全ケースで `resolved_mode=exact_compat` を返し、ANN 実行カウンタまたは監査ログが 0 件である
- [ ] `exact_compat` / `hnsw_ann` 双方で `corpusId/namespace` フィルタ結果が一致する

---

### TASK-AGDB-057: Cypher relationship type filter 拡張

**トレーサビリティ**: REQ-AGDB-028 → DES-AGDB-020  
**パッケージ**: `src/query.rs`, `tests/cypher_conformance.rs`  
**種別**: backend  
**優先度**: P0  
**依存**: TASK-AGDB-021, TASK-AGDB-022, TASK-AGDB-023  
**見積**: 1.5h

**実装内容**:
- parser AST に `-[r:TYPE]->` 単一型フィルタを追加する
- planner/executor に type filter predicate を実装する
- 型未存在時 empty row set 挙動を固定する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] `MATCH (n)-[r:TYPE]->(m)` の型一致のみ返却される
- [ ] `OPTIONAL MATCH/WHERE/RETURN` 併用時もフィルタが維持される

---

### TASK-AGDB-058: Entity vectors +64K staged ingest 実装

**トレーサビリティ**: REQ-AGDB-030 → DES-AGDB-022  
**パッケージ**: `src/storage.rs`, `src/native_bench.rs`, `src/server.rs`  
**種別**: backend  
**優先度**: P0  
**依存**: TASK-AGDB-055, TASK-AGDB-056  
**見積**: 1.5h

**実装内容**:
- `EntityVectorImporter` staged ingest（validate -> index -> commit）を実装する
- インポート失敗時 rollback を実装する
- searchable count before/after を品質ゲートで検証する
- `P0-ENTITY-VECTOR-QUALITY-BENCH@1.0.0` のプロファイル定義（固定query set/判定規則）を実装する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] 追加 64K vectors が取り込み後の検索対象件数に反映される
- [ ] `P0-ENTITY-VECTOR-QUALITY-BENCH@1.0.0`（固定 dataset_hash/query_set_hash/seed）で `Recall@10 >= ingest前baseline - 0.005` を満たす

---

### TASK-AGDB-059: v0.3.0 ベンチ/品質ゲート実装

**トレーサビリティ**: REQ-AGDB-026, REQ-AGDB-027, REQ-AGDB-030 → DES-AGDB-018, DES-AGDB-019, DES-AGDB-022  
**パッケージ**: `src/native_bench.rs`, `.github/workflows`  
**種別**: infra  
**優先度**: P0  
**依存**: TASK-AGDB-054, TASK-AGDB-056, TASK-AGDB-058  
**見積**: 1.5h

**実装内容**:
- `P0-VECTOR-STORAGE-SIZE-BENCH@1.0.0` / `P0-ANN-HNSW-BENCH@1.0.0` / `P0-ENTITY-VECTOR-QUALITY-BENCH@1.0.0` の CI 実行配線を実装する
- benchmark report の artifact 保存としきい値判定（release gate fail-fast）を実装する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] 3つの benchmark report が CI artifact として保存される
- [ ] `P0-VECTOR-STORAGE-SIZE-BENCH@1.0.0` で削減率 < 66% の場合に CI release gate が失敗する
- [ ] `P0-ANN-HNSW-BENCH@1.0.0` で `Recall@10 < 0.95` または `P95短縮率 < 50%` の場合に CI release gate が失敗する
- [ ] `P0-ENTITY-VECTOR-QUALITY-BENCH@1.0.0` で `Recall@10 < ingest前baseline - 0.005` の場合に CI release gate が失敗する

---

### TASK-AGDB-060: v0.3.0 統合適合/回帰配線

**トレーサビリティ**: REQ-AGDB-026, REQ-AGDB-027, REQ-AGDB-028, REQ-AGDB-029, REQ-AGDB-030 → DES-AGDB-018, DES-AGDB-019, DES-AGDB-020, DES-AGDB-021, DES-AGDB-022  
**パッケージ**: `tests`, `spec/conformance`, `.github/workflows`  
**種別**: test  
**優先度**: P0  
**依存**: TASK-AGDB-055, TASK-AGDB-057, TASK-AGDB-059  
**見積**: 1.5h

**実装内容**:
- REQ-026..030 の統合回帰テストセットを追加する
- mode 境界（`exact_compat` vs `hnsw_ann`）の誤配線防止テストを追加する
- v0.3.0 release-block 条件を workflow に配線する

**受入基準**:
- [ ] テストが書かれている（Red）
- [ ] テストが通る（Green）
- [ ] リファクタリング済み（Blue）
- [ ] REQ-026..030 の acceptance が CI 上で機械検証される

## 13. v0.3.0 Delta カバレッジ（REQ 26/27/28/29/30）

| REQ | 対応TASK (v0.3.0 Delta) |
|---|---|
| REQ-AGDB-026 | TASK-AGDB-054, TASK-AGDB-059, TASK-AGDB-060 |
| REQ-AGDB-027 | TASK-AGDB-056, TASK-AGDB-059, TASK-AGDB-060 |
| REQ-AGDB-028 | TASK-AGDB-057, TASK-AGDB-060 |
| REQ-AGDB-029 | TASK-AGDB-055, TASK-AGDB-060 |
| REQ-AGDB-030 | TASK-AGDB-058, TASK-AGDB-059, TASK-AGDB-060 |

| DES | 対応TASK (v0.3.0 Delta) |
|---|---|
| DES-AGDB-018 | TASK-AGDB-054, TASK-AGDB-059, TASK-AGDB-060 |
| DES-AGDB-019 | TASK-AGDB-056, TASK-AGDB-059, TASK-AGDB-060 |
| DES-AGDB-020 | TASK-AGDB-057, TASK-AGDB-060 |
| DES-AGDB-021 | TASK-AGDB-055, TASK-AGDB-060 |
| DES-AGDB-022 | TASK-AGDB-058, TASK-AGDB-059, TASK-AGDB-060 |
