...
use aira_graphdb::audit::{NativeCrashAuditEvent, now_epoch_ms_string, validate_crash_event_required_fields};
use aira_graphdb::native_bench::{
    parse_native_soak_profile, run_native_soak_report, write_native_audit_artifact, write_native_soak_report,
};
...
fn writes_native_soak_artifacts_and_meets_gate() {
    ...
    write_native_soak_report(&report, SOAK_REPORT_PATH).expect("native soak report should be written");
    write_native_audit_artifact(&report, AUDIT_REPORT_PATH).expect("native audit artifact should be written");
