use aira_graphdb::conformance::{
    ...
    build_and_persist_neo4j_compat_conformance_report,
    load_neo4j_compat_required_spec, load_required_tests_spec,
    run_conformance_governance_checks, run_neo4j_compat_governance_checks,
};
...
fn neo4j_compat_required_spec_is_traceable() {
    ...
    assert!(spec.required_test_ids.iter().any(|id| id == "NEO4J-COMPAT-RUN-001"));
    assert!(
        spec.governance_checks
            .iter()
            .any(|check| check == "unsupported_clauses_return_unsupported_feature")
    );
    assert!(spec.governance_checks.iter().any(|check| check == "subquery_clauses_execute"));
}
