...
use aira_graphdb::watchdog::{WatchdogCrashReport, observe_child_exit_as_crash, write_watchdog_crash_report};
...
fn records_process_crash_for_kill_level_exit() {
    ...
    let event = observe_child_exit_as_crash(child, Some("req-kill-1".to_string()), started, Some("killed by watchdog test".to_string()))
    ...
    let report = WatchdogCrashReport { events: vec![event] };
